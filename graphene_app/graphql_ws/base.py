...
class BaseConnectionContext(object):
    ...
    def remove_operation(self, op_id):
        print(f"remove operation ---------------------------------> {self.operations}")
        try:
    ...
    def unsubscribe_all(self):
        print("unsubscribe_all websocket")
...
class BaseSubscriptionServer(object):
    ...
    def __init__(self, schema, keep_alive=False):
        self.schema = schema
...
def get_index(main_string, to_find, second=False):
    count = 0
    for index, letters in enumerate(main_string):
        if letters == to_find:
            count = count + 1
            if count == 1 and second is False:
                return index
            if count == 2 and second is True:
                return index
